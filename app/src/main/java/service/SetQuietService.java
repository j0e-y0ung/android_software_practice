			...
			public void run() {
				...
				for(int day=0;day<7;day++){
					for(int row=0;row<12;row++){
						...
						temp[day][row][0] = cursor[day].getString(cursor[day].getColumnIndex("time1"));
						temp[day][row][1] = cursor[day].getString(cursor[day].getColumnIndex("time2"));
						if(!temp[day][row][0].equals(""))
							temp[day][row][0] = temp[day][row][0].substring(temp[day][row][0].indexOf(":")+2,temp[day][row][0].indexOf(":")+7);
						//Log.d(TestApp,"----------"+temp[day][row][0]);
						//Log.d(TestApp,"----------"+temp[day][row][1]);
					}
				}
				...
				for(int j=0;j<12;j++){
					...
					if(temp[currentDay][j][0].equals(ShareMethod.getTime()) ){
						audioManager.setStreamVolume(STREAM_RING, 0, 0);
						audioManager.setStreamVolume(STREAM_ALARM, 0, 0);
						audioManager.setStreamVolume(STREAM_MUSIC, 0, 0);
//						System.out.println("class is on");
					}
					if(temp[currentDay][j][1].equals(ShareMethod.getTime())){
						audioManager.setStreamVolume(STREAM_RING, 3, 0);
						audioManager.setStreamVolume(STREAM_ALARM, 3, 0);
						audioManager.setStreamVolume(STREAM_MUSIC, 3, 0);
